...
$(document).on('click', '.prop-item', function (e) {
    ...
    hash = $(this).data("hash");
    $.post(`http://exter-objectspawner/openProps`, JSON.stringify({itemName:itemName,propName:propName,hash:hash}), function (x) {
        if (x=='openProps') {
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape') {
        $("body").fadeOut(500);
        $.post(`http://exter-objectspawner/close`, JSON.stringify({ }), function (x) {});
    }
});
...
$(document).on('click', '.prop-settings', function (e) {
    ...
    if (dataType=="delete") {
        $.post(`http://exter-objectspawner/deleteProp`, JSON.stringify({ }), function (x) {});
    }
...
$(document).on('click', '.save-build-button', function (e) {
    ...
    $.post(`http://exter-objectspawner/saveBuild`, JSON.stringify({ }), function (x) {});
})
